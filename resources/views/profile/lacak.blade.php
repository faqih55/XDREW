...
<div class="max-w-5xl mx-auto space-y-8 animate-smooth-reveal">
    <header class="mb-8">
        <h1 class="font-['Outfit'] text-3xl font-extrabold text-[#0A1612] mb-2 tracking-tight">Lacak Pesanan</h1>
        <p class="text-[#0A1612]/70 text-sm">Cari atau pilih nomor pesanan Anda untuk melacak pengiriman secara real-time.</p>
    </header>
    ...
    <div class="bg-white/40 backdrop-blur-md rounded-[2rem] p-8 border border-white/60 shadow-lg space-y-8">
        ...
        <form action="{{ route('profile.lacak') }}" method="GET" class="flex flex-col md:flex-row gap-4">
            <div class="flex-grow group">
                <label class="text-xs text-[#0A1612]/70 block mb-2 font-bold group-focus-within:text-[#10b981] transition-colors">Pilih Pesanan Anda</label>
                <select name="order_id" class="w-full bg-white/60 border border-white/80 rounded-xl px-4 py-3 text-[#0A1612] focus:bg-white focus:outline-none focus:border-[#10b981] focus:ring-1 focus:ring-[#10b981] transition-all duration-300">
        ...
        @if($searchedOrder)
            ...
            <div class="border-t border-[#1A2E26]/10 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div class="lg:col-span-7 space-y-6">
                    <h3 class="font-['Outfit'] text-lg font-bold text-[#0A1612]">Status Perjalanan</h3>
                    <div class="flex flex-col gap-10 pl-2">
                        <!-- Step 1 -->
                        <div class="timeline-step relative active flex gap-5">
                            <div class="relative z-10 w-6 h-6 rounded-full bg-[#10b981] border-4 border-[#F9FAFB] flex items-center justify-center shadow-md">
                            ...
                            <div>
                                ...
                                <p class="text-[#0A1612]/50 text-[10px] font-medium">{{ isset($searchedOrder->created_at) ? \Carbon\Carbon::parse($searchedOrder->created_at)->format('d M Y • H:i') : (isset($searchedOrder->tanggal_pesanan) ? \Carbon\Carbon::parse($searchedOrder->tanggal_pesanan)->format('d M Y • H:i') : date('d M Y • H:i')) }}</p>
                                <p class="text-[#0A1612]/60 mt-1 text-xs leading-relaxed">Pesanan Anda telah diterima sistem kami.</p>
                            </div>
                        </div>
                        ...
                        <div class="timeline-step relative {{ $step2_active ? 'active' : '' }} flex gap-5">
                            ...
                            @else
                                <div class="relative z-10 w-6 h-6 rounded-full bg-white border-4 border-[#F9FAFB] shadow-sm flex items-center justify-center">
                                    <div class="w-1.5 h-1.5 rounded-full bg-gray-300"></div>
                                </div>
                            @endif
                            <div>
                                <h4 class="text-sm font-bold {{ $step2_active ? 'text-[#10b981]' : 'text-[#0A1612]/40' }} mb-1 uppercase tracking-wider">Pembayaran Dikonfirmasi</h4>
                                <p class="text-[#0A1612]/50 text-[10px] font-medium">
                                ...
                                <p class="text-[#0A1612]/60 mt-1 text-xs leading-relaxed">Verifikasi transaksi pembayaran Anda berhasil.</p>
                            </div>
                        </div>
                        ...
                        <div class="timeline-step relative {{ $step3_active ? 'active' : '' }} flex gap-5">
                            ...
                            @endif
                            <div>
                                <h4 class="text-sm font-bold {{ $step3_active ? 'text-[#10b981]' : 'text-[#0A1612]/40' }} mb-1 uppercase tracking-wider">Sedang Diproses</h4>
                                <p class="text-[#0A1612]/50 text-[10px] font-medium">
                                    {{ $step3_active ? \Carbon\Carbon::parse($searchedOrder->updated_at)->format('d M Y • H:i') : 'Menunggu' }}
                                </p>
                                <p class="text-[#0A1612]/60 mt-1 text-xs leading-relaxed">Item Anda sedang dikemas di gudang utama kami.</p>
                            </div>
                        </div>
                        ...
                        <div class="timeline-step relative {{ $step4_active ? 'active' : '' }} flex gap-5">
                            ...
                            @endif
                            <div>
                                <h4 class="text-sm font-bold {{ $step4_active ? 'text-[#10b981]' : 'text-[#0A1612]/40' }} mb-1 uppercase tracking-wider">Dikirim</h4>
                                <p class="text-[#0A1612]/50 text-[10px] font-medium">
                                ...
                                <p class="text-[#0A1612]/60 mt-1 text-xs leading-relaxed">Paket telah diserahkan kepada kurir pengiriman.</p>
                            </div>
                        </div>
                        ...
                        <div class="timeline-step relative {{ $step5_active ? 'active' : '' }} flex gap-5">
                            ...
                            @endif
                            <div>
                                <h4 class="text-sm font-bold {{ $step5_active ? 'text-[#10b981]' : 'text-[#0A1612]/40' }} mb-1 uppercase tracking-wider">Sampai di Tujuan</h4>
                                <p class="text-[#0A1612]/50 text-[10px] font-medium">
                                ...
                                <p class="text-[#0A1612]/60 mt-1 text-xs leading-relaxed">Paket telah sampai di alamat tujuan pengiriman.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-5 space-y-6">
                    <div class="bg-white/60 backdrop-blur-md border border-white/80 p-6 rounded-2xl space-y-4 shadow-sm">
                        <h3 class="font-['Outfit'] text-sm font-bold text-[#0A1612] uppercase tracking-wider">Detail Pengiriman</h3>
                        <div class="space-y-3">
                            <div class="flex justify-between border-b border-[#1A2E26]/5 pb-2 text-xs">
                                <span class="text-[#0A1612]/60">Kurir</span>
                                <span class="font-bold text-[#0A1612]">{{ $searchedOrder->pengiriman->KURIR ?? $searchedOrder->pengiriman->kurir ?? 'JNE / J&T / Sicepat' }}</span>
                            </div>
                            <div class="flex justify-between border-b border-[#1A2E26]/5 pb-2 text-xs">
                                <span class="text-[#0A1612]/60">No. Resi</span>
                            ...
                            <div class="flex justify-between pb-1 text-xs">
                                <span class="text-[#0A1612]/60">Status</span>
        ...
        @elseif($orderId)
            <div class="text-center py-10">
                <p class="text-[#0A1612]/50 text-sm">Pilih pesanan Anda di atas untuk melacak.</p>
            </div>
        @else
            <div class="text-center py-12 bg-white/20 border border-dashed border-[#1A2E26]/10 rounded-2xl">
                <span class="material-symbols-outlined text-4xl text-[#0A1612]/30 mb-2">local_shipping</span>
                <p class="text-[#0A1612]/60 text-sm font-semibold">Belum Ada Pesanan yang Dilacak</p>
                <p class="text-[#0A1612]/40 text-xs mt-1">Silakan pilih nomor pesanan pada dropdown untuk melihat status pengiriman.</p>
            </div>
        @endif
