...
<div class="p-4 md:p-8 space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-black/5 pb-6">
        <div>
            <h1 class="text-3xl font-extrabold text-[#1A2E26] tracking-tight">Detail <span class="text-[#10b981]">Pelanggan.</span></h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white/60 rounded-2xl border border-black/5 p-8 flex flex-col items-center gap-4 text-center shadow-sm">
            <div class="w-24 h-24 rounded-3xl bg-[#10b981]/10 border-2 border-[#10b981]/30 flex items-center justify-center text-[#10b981] font-black text-4xl shadow-lg shadow-[#10b981]/20">
                {{ strtoupper(substr($nama, 0, 1)) }}
            </div>
            <div>
                <h2 class="text-xl font-extrabold text-[#1A2E26]">{{ $nama }}</h2>
            ...
            @if($joinAt)
                <div class="w-full pt-4 border-t border-black/5">
                    ...
                    <p class="text-[#1A2E26] font-semibold text-sm">{{ \Carbon\Carbon::parse($joinAt)->format('d F Y') }}</p>
                </div>
            @endif
        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white/60 rounded-2xl border border-black/5 overflow-hidden shadow-sm">
                <div class="px-6 py-4 border-b border-black/5 flex items-center gap-2 bg-white/40">
                    ...
                    <h3 class="font-bold text-[#1A2E26] uppercase tracking-wider text-sm">Informasi Pribadi</h3>
                </div>
                <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        <p class="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Nama Lengkap</p>
                        <p class="text-[#1A2E26] font-semibold text-sm">{{ $nama }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Email</p>
                        <p class="text-[#1A2E26] font-semibold text-sm break-all">{{ $email }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Nomor Telepon</p>
                        <p class="text-[#1A2E26] font-semibold text-sm">{{ $telp }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Bergabung</p>
                        <p class="text-[#1A2E26] font-semibold text-sm">{{ $joinAt ? \Carbon\Carbon::parse($joinAt)->format('d M Y') : '-' }}</p>
                    </div>
                    <div class="sm:col-span-2">
                        <p class="text-[10px] text-slate-500 uppercase tracking-widest mb-1">Alamat</p>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="bg-white/60 rounded-2xl border border-black/5 p-5 flex flex-col items-start gap-2 shadow-sm">
                    ...
                    <p class="text-2xl font-extrabold text-[#1A2E26]">
                ...
                <div class="bg-white/60 rounded-2xl border border-black/5 p-5 flex flex-col items-start gap-2 shadow-sm">
                    ...
                    <p class="text-lg font-extrabold text-[#1A2E26]">
