            ...
            <form action="{{ route('search') }}" method="GET" class="flex flex-col gap-3">
                <div class="relative flex items-center bg-white/40 shadow-[inset_0_2px_4px_rgba(255,255,255,0.6),0_4px_10px_rgba(98,124,112,0.05)] p-2 rounded-[1.5rem] border border-white/50 focus-within:border-[#10b981]/40 focus-within:bg-white/70 transition-all duration-500 group">
                    ...
                    <input type="text"
                           ...
                           class="w-full bg-transparent text-[#1A2E26] text-[15px] font-semibold outline-none placeholder-gray-400 font-['Poppins'] py-2.5 pl-14 pr-[100px]"
                           autocomplete="off">
                ...
                <div class="flex items-center justify-between px-4 pb-1">
                    <div class="flex gap-2 items-center hidden sm:flex">
                        <span class="text-[9px] uppercase tracking-widest text-[#1A2E26]/60 font-bold">Populer:</span>
                        <a href="{{ route('search', ['query' => 'Kemeja']) }}" class="text-[10px] font-medium text-[#1A2E26]/80 hover:text-[#10b981] px-2.5 py-1 rounded-md bg-white/40 hover:bg-white/60 transition-colors border border-white/40 hover:border-[#10b981]/30">Kemeja</a>
                        <a href="{{ route('search', ['query' => 'Celana']) }}" class="text-[10px] font-medium text-[#1A2E26]/80 hover:text-[#10b981] px-2.5 py-1 rounded-md bg-white/40 hover:bg-white/60 transition-colors border border-white/40 hover:border-[#10b981]/30">Celana</a>
                        <a href="{{ route('search', ['query' => 'Jaket']) }}" class="text-[10px] font-medium text-[#1A2E26]/80 hover:text-[#10b981] px-2.5 py-1 rounded-md bg-white/40 hover:bg-white/60 transition-colors border border-white/40 hover:border-[#10b981]/30">Jaket</a>
                    </div>
                    <p class="text-[10px] text-[#1A2E26]/60 font-['Poppins'] ml-auto">
                        Tekan <kbd class="px-1.5 py-0.5 rounded bg-white/60 border border-white/80 text-[#1A2E26]/80 font-mono shadow-sm">ESC</kbd>
                    </p>
                </div>
            </form>
        </div>
