        ...
        <div x-show="notifOpen"
             ...
             class="absolute top-[calc(100%+16px)] left-4 right-4 md:left-1/2 md:right-auto md:-translate-x-1/2 max-w-[380px] mx-auto md:mx-0 p-4 glass-dropdown rounded-[2rem] z-50 shadow-[0_20px_50px_rgba(0,0,0,0.8)] pointer-events-auto"
             style="display:none;">
            ...
            <div class="flex flex-col gap-1.5 max-h-[320px] overflow-y-auto pr-1 custom-scrollbar">
                ...
                @if($notifList->count() > 0)
                    @foreach($notifList as $notif)
                        ...
                        {{-- Each notification is a form POST to mark-as-read then redirect --}}
                        <div class="relative group/notif">
                            <a href="{{ $markReadUrl }}"
                               class="flex gap-3 p-3 rounded-2xl transition-all duration-200 border
                                ...
                                {{-- Icon --}}
                                <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 transition-transform duration-200 group-hover/notif:scale-110"
